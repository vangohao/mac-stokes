...
int residual(int n, int level, dtype ** u, dtype ** v, dtype ** p, dtype ** f, dtype **g, dtype * b, dtype * t, dtype * l, dtype * r, dtype ** rf, dtype ** rg, dtype ** rdiv, dtype * r0, dtype * r1)
{
    ...
    for(int i = 1; i< n;i++)
    {
        rf[i][0] = checkf(level,i,0) + (- (p[i][0] - p[i - 1][0]) * h + b[i] * h + u[i][1] + u[i + 1][0] + u[i - 1][0] - u[i][0] * 3) / (h * h);
    }
    for(int j = 1; j < n - 1; j++)
    {
        for(int i = 1; i < n; i++)
        {
            rf[i][j] = checkf(level,i,j) + ( - (p[i][j] - p[i -1][j]) * h + u[i+1][j] + u[i-1][j] + u[i][j+1] + u[i][j-1] - 4 * u[i][j]) / (h * h);
        }
    }
    // j = n - 1;
    for(int i = 1; i< n;i++)
    {
        rf[i][n-1] = checkf(level,i,n-1) + (- (p[i][n-1] - p[i - 1][n-1]) * h + t[i] * h + u[i][n-2] + u[i + 1][n-1] + u[i - 1][n-1] - 3 * u[i][n - 1]) / (h * h);
    }
    ...
    for(int i = 1; i< n;i++)
    {
        rg[0][i] = checkg(level, 0, i) + (- (p[0][i] - p[0][i - 1]) * h + l[i] * h + v[1][i] + v[0][i + 1] + v[0][i - 1] - 3 * v[0][i]) / (h * h);
    }
    for(int j = 1; j < n - 1; j++)
    {
        for(int i = 1; i < n; i++)
        {
            rg[j][i] = checkg(level, j,i) + (- (p[j][i] - p[j][i -1]) * h + v[j][i+1] + v[j][i-1] + v[j+1][i] + v[j-1][i] - 4 * v[j][i]);
        }
    }
    // j = n - 1;
    for(int i = 1; i< n;i++)
    {
        rg[n-1][i] = checkg(level, n-1, i) + (- (p[n-1][i] - p[n-1][i - 1]) * h + r[i] * h + v[n-2][i] + v[n-1][i + 1] + v[n-1][i - 1] - 3 * v[n-1][i]) / (h * h);
    }
