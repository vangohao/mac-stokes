...
int residual_uv(int n, int level, dtype ** u, dtype ** v, dtype ** f, dtype **g, dtype ** rf, dtype ** rg, dtype * r0)
{
    ...
    for(int i = 1; i< n;i++)
    {
        rf[i][0] = f[i][0] + (u[i][1] + u[i + 1][0] + u[i - 1][0] - u[i][0] * 3) / (h * h);
    }
    for(int j = 1; j < n - 1; j++)
    {
        for(int i = 1; i < n; i++)
        {
            rf[i][j] = f[i][j] + (u[i+1][j] + u[i-1][j] + u[i][j+1] + u[i][j-1] - 4 * u[i][j]) / (h * h);
        }
    }
    // j = n - 1;
    for(int i = 1; i< n;i++)
    {
        rf[i][n-1] = f[i][n-1] + (u[i][n-2] + u[i + 1][n-1] + u[i - 1][n-1] - 3 * u[i][n - 1]) / (h * h);
    }
    ...
    for(int i = 1; i< n;i++)
    {
        rg[0][i] = g[0][i] + (v[1][i] + v[0][i + 1] + v[0][i - 1] - 3 * v[0][i]) / (h * h);
    }
    for(int j = 1; j < n - 1; j++)
    {
        for(int i = 1; i < n; i++)
        {
            rg[j][i] = g[j][i] + (v[j][i+1] + v[j][i-1] + v[j+1][i] + v[j-1][i] - 4 * v[j][i]) / (h * h);
        }
    }
    // j = n - 1;
    for(int i = 1; i< n;i++)
    {
        rg[n-1][i] = g[n-1][i] + (v[n-2][i] + v[n-1][i + 1] + v[n-1][i - 1] - 3 * v[n-1][i]) / (h * h);
    }
